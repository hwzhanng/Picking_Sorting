...
Features:
- Stateless, pure-functional design for JAX JIT compilation
- Uses mjx.Data and mjx.Model for GPU-accelerated physics
- PID control integrated via pid.py
- IK base control via ik_base.py
- Designed for vmap across parallel environments

Key Design:
- No self references - model/data passed explicitly
- State containers (NamedTuples) for PID and other controllers
...
- Viewer functionality is not available
  → Use CPU MuJoCo for visualization
"""
...
from gym_dcmm.utils.pid import (
    PIDState, PIDParams,
    create_pid_state, create_pid_params,
    pid_step
)
from gym_dcmm.utils.ik_pkg.ik_base import (
