...
- **📁 完全独立的训练流程**: Stage 1 和 Stage 2 拥有独立的文件结构,便于调试和修改
...
### 1. 文件系统重构 (2025-12)
**完全分离 Stage 1 和 Stage 2**,实现独立开发:
- ✅ 独立的环境: `gym_dcmm/envs/stage1/` 和 `gym_dcmm/envs/stage2/`
- ✅ 独立的算法: `gym_dcmm/algs/ppo_dcmm/stage1/` 和 `gym_dcmm/algs/ppo_dcmm/stage2/`
- ✅ 独立的训练脚本: `train_stage1.py` 和 `train_stage2.py`
- ✅ 独立的配置: `configs/config_stage1.yaml` 和 `configs/config_stage2.yaml`
- ✅ 标准化命名: 所有类名遵循 `*Stage1` / `*Stage2` 规范
...
### 2. 训练第二阶段:抓取 (Catching / Stage 2)
使用预训练的追踪模型,训练灵巧手进行抓取。

**注意**: 请确保 `configs/config_stage2.yaml` 中的 `checkpoint_tracking` 指向你最好的第一阶段模型路径。
...
- 模型加载路径 (`checkpoint_tracking`)
- WandB 实验跟踪配置
- 默认训练配置: `train/stage1/PPO_Stage1.yaml`
...
- WandB 实验跟踪配置
- 默认训练配置: `train/stage2/PPO_Stage2.yaml`
...
```
catch_it/
├── train_stage1.py            # Stage 1 训练脚本 (Tracking)
├── train_stage2.py            # Stage 2 训练脚本 (Catching)
├── test_env.py                # 环境测试脚本
...
│   └── train/
│       ├── stage1/
│       │   └── PPO_Stage1.yaml    # Stage 1 PPO 参数
│       └── stage2/
│           └── PPO_Stage2.yaml    # Stage 2 PPO 参数
├── gym_dcmm/                  # 环境包
│   ├── envs/
│   │   ├── stage1/            # Stage 1 环境目录
│   │   │   ├── DcmmVecEnvStage1.py      # Stage 1 主环境
│   │   │   └── RewardManagerStage1.py   # Stage 1 奖励管理器
│   │   ├── stage2/            # Stage 2 环境目录
│   │   │   ├── DcmmVecEnvStage2.py      # Stage 2 主环境
│   │   │   └── RewardManagerStage2.py   # Stage 2 奖励管理器
│   │   ├── observation_manager.py   # 观测处理 (共享)
│   │   ├── control_manager.py       # 控制管理 (共享)
│   │   ├── randomization_manager.py # 场景随机化 (共享)
│   │   ├── render_manager.py        # 渲染管理 (共享) 
│   │   └── constants.py             # 常量定义 (共享)
│   ├── agents/MujocoDcmm.py   # MuJoCo 接口 + PID 控制
│   ├── algs/ppo_dcmm/         # PPO 算法实现
│   │   ├── stage1/            # Stage 1 算法目录
│   │   │   ├── PPO_Stage1.py        # Stage 1 PPO 训练
│   │   │   └── ModelsStage1.py      # Stage 1 网络(CNN+MLP)
│   │   ├── stage2/            # Stage 2 算法目录
│   │   │   ├── PPO_Stage2.py        # Stage 2 PPO 训练
│   │   │   └── ModelsStage2.py      # Stage 2 网络
│   │   ├── experience.py      # 经验缓冲 (共享)
│   │   └── utils.py           # 工具函数 (共享)
│   └── utils/                 # 工具函数
│       ├── util.py            # 通用工具
│       ├── pid.py             # PID 控制器
...
### 实现机制
1. **训练脚本** (`PPO_Stage1.py`):
   - 每次收集经验前,调用 `env.set_global_step(current_step)`
   
2. **环境调度器** (`DcmmVecEnvStage1.update_curriculum_difficulty()`)
   ```python
...
3. **奖励管理器** (`RewardManagerStage1.compute_reward()`):
...
### Q5: Stage 1 和 Stage 2 的区别?
**A**: 
- **Stage 1**: 训练底盘+机械臂,手掌固定张开姿态
- **Stage 2**: 底盘+机械臂由预训练模型控制(冻结),仅训练灵巧手
- 两个阶段现在拥有**完全独立的文件结构**,可以分别修改而不影响彼此
